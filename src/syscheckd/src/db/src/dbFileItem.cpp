...
void FileItem::createFimEntry()
{
    ...
    data->mtime = m_time;
    data->inode = m_inode;
    std::strncpy(data->hash_md5, m_md5.c_str(), sizeof(data->hash_md5 - 1));
    std::strncpy(data->hash_sha1, m_sha1.c_str(), sizeof(data->hash_sha1 - 1));
    std::strncpy(data->hash_sha256, m_sha256.c_str(), sizeof(data->hash_sha256 - 1));
    data->mode = m_mode;
    data->last_event = m_lastEvent;
    data->dev = m_dev;
    data->scanned = m_scanned;
    data->options = m_options;
    std::strncpy(data->checksum, m_checksum.c_str(), sizeof(data->checksum - 1));
