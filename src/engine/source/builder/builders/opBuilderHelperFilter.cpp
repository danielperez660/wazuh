...
types::Lifter opBuilderHelperExists(const DocumentValue & def)
{
    ...
    // Check parameters
    std::vector<std::string> parameters{utils::string::split(def.MemberBegin()->value.GetString(), '/')};
...
bool opBuilderHelperStringComparison(const std::string key, char op, types::Event & e,
                                     std::optional<std::string> refValue, std::optional<std::string> value)
{
...
types::Lifter opBuilderHelperStringNE(const DocumentValue & def)
{
    ...
    {
        // Append rxcpp operation
        return o.filter([key, refValue, value](types::Event e)
                        { return opBuilderHelperStringComparison(key, '!', e, refValue, value); });
    };
}
...
bool opBuilderHelperIntComparison(const std::string field, char op, types::Event & e,
                                  std::optional<std::string> refValue, std::optional<int> value)
{
...
types::Lifter opBuilderHelperIntEqual(const types::DocumentValue & def)
{
    ...
    std::optional<int> value = valuestr.has_value() ? std::optional<int>{std::stoi(valuestr.value())} : std::nullopt;

    // Return Lifter
    return [field, refValue, value](types::Observable o)
    {
        // Append rxcpp operation
        return o.filter([=](types::Event e) { return opBuilderHelperIntComparison(field, '=', e, refValue, value); });
    };
}
...
types::Lifter opBuilderHelperRegexMatch(const types::DocumentValue & def)
{
    ...
    if (!regex_ptr->ok())
    {
        const std::string err = "Error compiling regex '" + parameters[1] + "'. " + regex_ptr->error();
...
types::Lifter opBuilderHelperIPCIDR(const types::DocumentValue & def)
{
    ...
    {
        // Append rxcpp operations
        return o.tap([=](types::Event e) { std::cerr << "    {" << field << ": ipcdr got" << std::endl; })
            .filter(
                [=](types::Event e)
                {
                    // TODO Remove try catch
            ...
            .tap([=](types::Event e) { std::cerr << "    {" << field << ": ipcdr sent" << std::endl; });
    };
}
